...
(function() {
  ...
  (function(root, factory) {
    ...
    var Attributed, Computing, EObject, Element, Evented, Hiding, Model, ModelMask, Router, Stateful, View, ViewCollection, ViewModel, array, create, isArray, load;
    isArray = function(a) {
    ...
    create = function() {
      var component, components, obj, _i, _len;
      obj = arguments[0], components = 2 <= arguments.length ? __slice.call(arguments, 1) : [];
    ...
    array = function(arr) {
      var item, orig, _i, _len;
      Evented("Model").call(arr);
      arr.on("add", function(item) {
        if (item.attach) {
          return item.attach(arr, true, true);
        }
      });
      orig = Array.prototype;
      arr.pop = function() {
        var result;
        result = orig.pop.apply(this, arguments);
        this.trigger("remove", result);
        return result;
      };
      arr.push = function() {
        var item, items, result, _i, _len;
        items = 1 <= arguments.length ? __slice.call(arguments, 0) : [];
        result = orig.push.apply(this, arguments);
      ...
      arr.shift = function() {
        var result;
        result = orig.shift.apply(this, arguments);
        this.trigger("remove", result);
        return result;
      };
      arr.unshift = function() {
        var item, items, result, _i, _len;
        items = 1 <= arguments.length ? __slice.call(arguments, 0) : [];
        result = orig.unshift.apply(this, arguments);
        for (_i = 0, _len = items.length; _i < _len; _i++) {
          item = items[_i];
          this.trigger("add", item, true);
        }
        return result;
      };
      arr.splice = function() {
        var item, result, _i, _len;
        result = orig.splice.apply(this, arguments);
        this.trigger("reset");
    ...
    Attributed = function(attributes) {
      ...
      return function() {
        ...
        return this.set = function(key, value, rebuilding) {
          if ((this.validate && this.validate(key, value)) || !this.validate) {
            attributes[key] = value;
            this.trigger("change:" + key, value, attributes, rebuilding);
            return this.trigger("change", key, value, attributes, rebuilding);
          }
        };
      };
    };
    Element = function(tag, html) {
      ...
      return function() {
        var ev, rebuild, runAction, _i, _len, _ref,
          _this = this;
        rebuild = function() {
          var attr, attrBinding, attrBinds, attributedElements, bind, bindElements, classBinding, classBinds, classyElements, conditions, el, func, name, outletElements, parts, ternery, _i, _j, _k, _l, _len, _len1, _len2, _len3, _len4, _len5, _m, _n, _name, _name1;
          ...
          for (_k = 0, _len2 = attributedElements.length; _k < _len2; _k++) {
            ...
            for (_l = 0, _len3 = attrBinds.length; _l < _len3; _l++) {
              ...
              if (attributes[_name = ternery[0]] == null) {
                attributes[_name] = [];
              }
          ...
          for (_m = 0, _len4 = classyElements.length; _m < _len4; _m++) {
            ...
            for (_n = 0, _len5 = classBinds.length; _n < _len5; _n++) {
              ...
              if (classes[_name1 = ternery[0]] == null) {
                classes[_name1] = [];
              }
        ...
        this.insertInto = function(view, outlet, pre) {
          var el;
          if (typeof view === "string") {
            el = document.querySelector(view);
            return el.appendChild(element);
          } else {
            return view.insert(element, outlet, pre);
          }
        };
        this.insert = function(view, outlet, pre) {
          var el;
          if (view.insertInto) {
            return this.insertInto(this, outlet);
          } else {
            el = outlets[outlet];
            if (pre && el.firstChild) {
              return el.insertBefore(view, el.firstChild);
            } else {
              return el.appendChild(view);
            }
          }
        };
    ...
    Model = (function(_super) {
      ...
      Model.init = function(func) {
        var destroy, masks, models, queues, raws, store, update,
          _this = this;
        this.is(Evented("Model"));
        models = [];
        raws = [];
        ...
        create = function(item) {
          return new _this(item, true);
        };
        update = function(item) {
          var key, model, val, _results;
          model = _this.find(item.id);
          _results = [];
          for (key in item) {
            val = item[key];
            _results.push(model.set(key, val, true));
          }
          return _results;
        };
        destroy = function(item) {
          return _this.remove(item.id, true);
        };
        ...
        this.store = function(newStore, name) {
          if (newStore) {
            store = newStore(name, create, update, destroy);
          }
          this.fetch();
          return store;
        };
        ...
        this.on("change:model", function(key, value, model, rebuilding) {
          var delta, _name;
          if (!rebuilding) {
            delta = {};
            delta[key] = value;
            if (queues[_name = model.id] == null) {
              queues[_name] = [];
            }
        ...
        this.remove = function(model, raw, rebuilding) {
          var i, id, index, _i, _len, _name;
          ...
          if (!rebuilding) {
            if (queues[_name = id = raws[index].id] == null) {
              queues[_name] = [];
            }
            queues[id].push({
              action: "destroy",
    ...
    ViewCollection = (function(_super) {
      ...
      function ViewCollection(options) {
        ...
        viewmodels = {};
        this.on("add:Model", function(model, pre) {
          var view;
          ...
          if (_this.parent) {
            return view.insertInto(_this.parent, _this.outlet, pre);
          }
        });
